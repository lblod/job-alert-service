 ...
 */
export async function createAlertForJob(job, rateLimitContext = {}) {
  ...
  const previousSuppressed = rateLimitContext.previousSuppressed || [];
  ...
  const content = template({
    ...
    isLastAllowed: rateLimitContext.isLastAllowed || false,
    rateLimitMax: rateLimitContext.rateLimitMax,
    rateLimitWindowHours: rateLimitContext.rateLimitWindowHours,
    hasPreviousSuppressed: previousSuppressed.length > 0,
    previousSuppressed,
    previousSuppressedCount: previousSuppressed.length,
  });
    ...
    INSERT DATA {
      GRAPH <${config.graph.email}> {
        ${sparqlEscapeUri(uri)} a nmo:Email ;
          ...
          nmo:isPartOf ${sparqlEscapeUri(config.email.folder)} ;
